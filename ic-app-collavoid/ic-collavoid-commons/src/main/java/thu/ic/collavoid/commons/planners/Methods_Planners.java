...
//                    logger.warn("not with in {} constraint {}",additional_constraints.get(i).getType(),additional_constraints.get(i).toString());
                    return false;
                }
            }
            return true;
        ...
        static void createObstacleLine(Agent agent, Vector2 obst1, Vector2 obst2) {
            ...
            if (dist == Vector2.absSqr(Vector2.minus(agent.position.getPos(), obst1))) {
                ...
                List<Vector2> obst = new ArrayList<>();
                obst.add(Vector2.minus(obst1, position_obst));
                obst.add(Vector2.minus(obst2, position_obst));
...
//        System.out.println(agent.Name+"************************* dif ang: "+dif_ang+"; speed ang "+speed_ang+"; heading "+agent.position.getHeading());
    ...
    public static List<PoseStamped_> makePlan(final PoseStamped_ start, final PoseStamped_ goal) {
        ...
        List<PoseStamped_> plan = new ArrayList<>();
        plan.clear();
        plan.add(start);
