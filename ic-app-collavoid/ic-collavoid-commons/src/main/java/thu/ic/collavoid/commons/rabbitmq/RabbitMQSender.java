...
public class RabbitMQSender {
    ...
    public void close() {
        LOG.debug("Closing channel to exchange {}", exchangeName);
        try {
        ...
        try {
            LOG.debug("closing connection to rabbitmq: " + connection);
            connection.close();
        } catch (Exception e) {
    ...
    public void open(String exchangeType) throws Exception {
        try {
            ...
            if (prefetchCount > 0) {
                LOG.debug("setting basic.qos / prefetch count to " + prefetchCount + " for " + exchangeName);
    ...
    private Connection createConnection() throws IOException {
        try {
            ...
            LOG.debug("connected to rabbitmq: " + connection + " for " + exchangeName);
            return connection;
        } catch (Exception e) {
            LOG.error("connection failed to rabbitmq: " + connection + " for " + exchangeName);
            return null;
        }
    }
