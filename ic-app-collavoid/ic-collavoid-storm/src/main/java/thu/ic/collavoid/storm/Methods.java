...
public class Methods {
    ...
    public static PoseShareMsg_ extractPoseShareMsgFromAgent(Agent agent) {
        ...
        List<Vector2> footprint = new ArrayList<>();
...
//            logger.info("pref vel {}",agent.prefVelociy.toString());
            forwardAgents(agent);
...
//            logger.info("new vel {}",agent.newVelocity.toString());
    ...
    private static void updateAllNeighbors(Agent agent) {
        for (Neighbor agent1 : agent.AgentNeighbors) {
            updateAgentState(agent1, false);
        }
        Collections.sort(agent.AgentNeighbors, new Comparators.NeighborDistComparator(agent.position.getPos()));
    }

    private static void updateAgentState(Neighbor agt, boolean self) {
        double time_dif;
        if (agt.getLastSeen() == 0) {
//            logger.info("is self {} last seen 0, first run",self);
            time_dif = 0;
        } else {
//            logger.info("is self {} not zero",self);
            time_dif = System.currentTimeMillis() - agt.getLastSeen();
        }
        time_dif = time_dif / 1000.0;
//        logger.info("{} time dif {}",agt.Name,time_dif);
...
//        logger.info("x position {}",agt.getBaseOdom().getPose().getPosition().getX());
//        logger.info("x dif {}",x_dif);
...
//                logger.info("{} reached goal x: {}, y {}, theta {}.",
//                        agent.id,
...
//        logger.info("pref vel: {}",currentContext.pref_vel.toString());
...
//            final Pose_ global_pose, final Twist_ robot_vel, double goal_th, Twist_ cmd_vel) {
...
//        logger.info("Moving to desired goal orientation, th cmd: %1$2f" + v_th_samp);
    ...
    private static void checkVelocityCommand(Agent agent) {
        ...
        if (agent.cmd_vel.getLinear().getX() == 0.0 &&
            ...
            logger.info(agent.id + ": Did not find a good vel, trying next waypoint!");
            ...
            if (agent.current_waypoint < agent.transformed_plan.size() - 1) {
                ...
                agent.transformed_plan.clear();
                logger.warn(agent.id + ": Can not reach the goal!");
            }
        } else {
